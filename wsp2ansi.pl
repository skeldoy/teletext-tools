...
my $file = "$ARGV[0]";
if ($ARGV[0] eq "") { die "Bruk filnavn på en WSP-fil som parameter"; }
...
@lines = split("", $data);
for ($k =0; $k < 40; $k++) { $header .= $lines[$k]; } #Peller ut headeren
for ($j = 40; $j < 961; $j ++) { $text .= "$lines[$j]"; } #Peller ut innholdet
for ($l=961;$l<=$#lines;$l++) { $footer .= $lines[$l]; } #Peller ut footeren

@otherLines = split("", $text); #splitter ut innholdet i en char-array


$tempcounter=0;
for ($i = 0; $i <= $#otherLines; $i++) {
	$line = $otherLines[$i];
	&printColours("$line"); # Konvertere WSP-farger til ANSI
	$tempcounter++;
	if ($tempcounter==40) { 
		$tempcounter=0; 
		print "\n"; } # Linjeskift for leselighet
}
